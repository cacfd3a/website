...
Custom keyboards will by default only send typed text to the [host application]({{site.urls.terminology}})'s text field, and will not detect if you try to type into a text field in the keyboard. This makes it hard to build search, AI prompting, etc. 
...
KeyboardKit Pro unlocks text input views that automatically register and unregister themselves as the main [text document proxy]({{site.urls.terminology}}) when they receive and lose focus.
